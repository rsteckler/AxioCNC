# Project Todos

This document consolidates all project todos from various sources.

## Before beta release
First use
stats
outline job envelope
machine signales and input signales (iosender, xyzab, hsreop)
coolant
total distance
tool wear (by travel / hours run)
modal groups (mcpdfupsc): (https://community.carbide3d.com/t/cncjs-feedrate-converted-to-mm-even-though-g20/27111)
feed, spindle, rapid (cool way to say go slow)
detemrine mediamtx runtime file locations like logs and config and make sure they all land inside the .axiocnc folder
settings export/import
docs
auto updates
- [ ] Fix bitsetter to measure first bit if it's not already stored (maybe handle initial M6?)
- [ ] Make sure continue on error works
- [ ] Test all button mappings on gamepad
- [ ] Add screenshots to website and readme



## Later
- [ ] Move the gcode into the server and abstract it with commands
- [ ] Add keyboard jogging (WASD)
- [ ] Add ortho viewing projection mode
- [ ] Widget system
- [ ] File management
spinup time for spindle before moving
i18n
anonymous usage data
add automatic pulling of bed size and speeds from $$
air cutting
direction change rate
queue volatility
Dead-motion ratio
retract inefficiency
